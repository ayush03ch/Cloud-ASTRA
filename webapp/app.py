...
def api_scan():
    ...
    try:
        ...
        if agent == 's3':
            ...
            # Route53-specific parameters
            route53_scope = data.get('route53_scope', 'all')
            route53_intent = data.get('route53_intent', '')
            
            route53_checks = data.get('route53_checks', {
                'dnssec': True,
                'query_logging': True,
                'health_checks': True,
                'caa_records': True,
            ...
            user_intent_input = {}
            if route53_intent:
                user_intent_input['_global_intent'] = route53_intent
            
            # Handle zone-specific intent
            route53_zone_intents = data.get('route53_zone_intents', {})
            if route53_zone_intents:
                user_intent_input.update(route53_zone_intents)
